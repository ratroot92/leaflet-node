...
userRouter.post(
  ...
  [
    ...
    check("birthday", "Birthday date is required")
      .notEmpty()
      .custom((date) => {
        if (!momentJS(date, "YYYY-MM-DD", true).isValid()) {
          throw new Error(
            "Birthday date should be in valid format YYYY-MM-DD "
          );
        }
        return true;
      }),
  ...
  (req, res) => {
    ...
    //TODO:Validation Successfull
      ...
      if (category) {
        ...
        signupData["signup_ip"] =
          req.headers["x-real-ip"] || req.connection.remoteAddress;
        signupData["avatar"] = 0;
        res.status(200).json(signupData);
        //TODO:Insert User
        var insertUser = DB.query(
          "INSERT INTO cb_users SET ?",
          signupData,
          function (err, result) {
            if (err) {
            ...
            //TODO:Insertion Successfull
              ...
              var insertUser = DB.query(
                "INSERT INTO cb_user_profile SET ?",
                profileData,
                function (err, result) {
                  if (err) {
                    res.status(400).json(err);
...
function userExists(mentionUsername) {
  return new Promise((resolve, reject) => {
    DB.query(
      "SELECT COUNT(*) AS total FROM cb_users WHERE   username=?",
      [mentionUsername],
...
function emailExists(mentionEmail) {
  return new Promise((resolve, reject) => {
    DB.query(
      "SELECT COUNT(*) AS total FROM cb_users WHERE   email=?",
      [mentionEmail],
      function (error, results, fields) {
        if (!error) {
          console.log("MENTION COUNT : " + results[0].total);
...
userRouter.get("/base/configss", (req, res) => {
  let configs = {
    ...
    allow_upload: "",
    allowed_types:
      "wmv,avi,divx,3gp,mov,mpeg,mpg,xvid,flv,asf,rm,dat,mp4,png,jpg",
    version: "2.0.1",
    version_type: "Alpha",
    ...
    pages: [
      ...
      },
      {
        ...
        page_title: "Help",
        page_content:
          '<span style="font-weight: bold;">How to use Deikho</span><br><ol><li>Articles will be written pretty soon</li></ol>',
      },
      {
        page_id: 7,
    ...
    paidConfigs: {
      ...
      fd_currency: "840",
      paypal_client_id:
        "AWFF4AUcHLZDLAOpfUo8f0VoJ8VkN3uXhwwrDpBoHV7ow4ojM5A-bY-JA4zf1TAwrf_DxHyr_u5wFzt5",
      paypal_secret:
        "EA2Q8W_fQottezQl0dA1vya8zJbzGbqMeLlI-MqTEhJWmekA6AEA8XixzMth0VvcgaXXJQ1orMF59QrH",
...
const activeSubscriptionExists = (userid, active) => {
  var signIn = DB.query(
    "SELECT COUNT(*) AS total FROM cb_paid_subscriptions WHERE userid = ? AND active = ?",
    [userid, active],
    function (err, result) {
      if (!error) {
        console.log("MENTION COUNT : " + results[0].total);
